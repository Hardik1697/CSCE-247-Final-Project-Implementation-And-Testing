...
public class Helper extends DataRemover {
  ...
  public void addSingleShowTime(String name, String type) {
    ...
    while (addSingleShowTimeBool()) {
      ...
      if (type.equalsIgnoreCase("Movie")) {
        showTimes.addShowTimeMovie(name, type, theater, time);
      }
      else if (type.equalsIgnoreCase("Play")) {
        showTimes.addShowTimePlay(name, type, theater, time);
      }
    }
  ...
  public void purchaseTicket(String email, String status, String type) {
    ...
    System.out.println("Would you like to view your Purchased Tickets? (Y or N)");
    String response = sc.nextLine();
   ...
   * @param email
   */
  public void accountSettings(String email) {
    boolean loop = true;
    while (loop) {
